...
/* ****************************************************************************
 % Dem_DTC_IsAgingForAllDTCsEnabled
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
 ...
 */
DEM_LOCAL_INLINE FUNC(boolean, DEM_CODE)
Dem_DTC_IsAgingForAllDTCsEnabled(                                                                                                /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  void)
{
...
# if ((DEM_FEATURE_NEED_READINESS_CALCULATION == STD_ON) && (DEM_CFG_SUPPORT_TYPE1_COMBINATION == STD_ON))
    ...
    if ( (Dem_Core_Event_GetEventReadinessGroup(EventId) != DEM_CFG_READINESS_NONE)
...
# if (DEM_CFG_SUPPORT_PENDING_STORAGE_FREEZE_FRAME == STD_ON)      
      if ((Dem_Cfg_IsObdIIExclusivelySupportedInVariant() == TRUE) && (Dem_UDSStatus_Test_CDTC(Dem_DTC_RetrieveStatus(EventId)) == FALSE))
      {
...
# if (DEM_CFG_SUPPORT_OBDII == STD_ON) && (DEM_CFG_SUPPORT_CONFIRMED_VISIBLE_FREEZE_FRAME == STD_ON)
      ...
      if ( (Dem_Cfg_IsObdIIExclusivelySupportedInVariant() == TRUE)
        && (Dem_UDSStatus_Test_CDTC(lOldDtcStatus) == FALSE)
...
# endif
  ...
  { /* Start Aging */
    ...
    Dem_Internal_AssertContinue((Dem_DTC_GetAgingCounter(EventId, Dem_MemoryEntry_GetId(lMemoryIndex)) == DEM_OPERATIONCYCLE_CYCLECOUNT_INVALID),\
...
# if (DEM_CFG_SUPPORT_TYPE1_COMBINATION == STD_ON)
    Dem_DTC_SetQualifyStatus(EventId, (Dem_Event_GetQualifyStatus(EventId) | ArgPtr->Specific.CycleQualifyDcy_Internal.EventQualificationStatus));
# endif
}
#endif
...
#if (DEM_CFG_SUPPORT_J1939 == STD_ON)
/* ****************************************************************************
 % Dem_Core_DTC_FindEventFromJ1939Dtc
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
)
{
  ...
  {
    lLowerLimit = Dem_MemState_MemoryGetFirstEvent(TranslatedMemoryId);
    lEventId = Dem_MemState_MemoryGetLastEvent(TranslatedMemoryId);
  }
  else
  {
...
/* ****************************************************************************
 % Dem_Core_DTC_FindEventFromUdsDtc
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
)
{
  ...
  else if (TranslatedMemoryId == DEM_CFG_MEMORYID_PERMANENT)
  {
    lLowerLimit = Dem_MemState_MemoryGetFirstEvent(DEM_CFG_MEMORYID_PERMANENT);
    lEventId = Dem_MemState_MemoryGetLastEvent(DEM_CFG_MEMORYID_PERMANENT);
  }
  else
  {
    lLowerLimit = DEM_EVENT_INVALID;
...
)
{
  ...
  {
    lReturnValue = (boolean)(Dem_GlobalState_ActivationMode_IsEventAndActivationMode4Active(EventId) == TRUE);
  }
  else
  {
    /* MISRA case */
  ...
  )
{
  ...
  if (Dem_Cfg_IsPermanentSupportedInVariant() == TRUE)
  {
    if (Dem_Cfg_IsObdIIOrObdOnUdsSupportedInVariant() == TRUE)
    {
      lReturnValue = Dem_Core_Event_TestMilAssigned(EventId);
    }
    else if (Dem_Cfg_IsWwhObdSupportedInVariant() == TRUE)
    {
...
# if (DEM_CFG_PROCESS_CDTC_STOREDONLY == STD_ON)
  if (Dem_DTC_TestEventStoresPermanentDTC(EventId) == TRUE)
  {
    if ( (Dem_Memory_TestEventStorageTriggerIsOnConfirmed(EventId) == TRUE)
      || (Dem_DTC_IsEventCombinationType2Enabled() == FALSE)
      || ( (Dem_UDSStatus_Test_CDTC(lArgs.Common.OldDtcStatus) == TRUE)
        && (Dem_UDSStatus_Test_CDTC(lArgs.Common.NewDtcStatus) == FALSE)))
    {
...
)
{
  ...
  Dem_Event_ExtendedStatus_ProcessPassedEffectsTPSLC(DTCContext);                                                                /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  ...
  if ((Dem_Cfg_EventUdsDtc(DTCContext->EventId) != DEM_CFG_DTC_UDS_INVALID)
    && (Dem_Conditions_StorageCondition_TestEventStorageConditionsFulfilled(DTCContext->EventId) == TRUE))                                  
  {
    Dem_Cfg_MemoryIndexType lMemoryIndex;
    Dem_Event_InternalStatusType lInternalStatus;
  ...
  )
{
  ...
  { /* Not stored yet - If no entry can be created we need to discard some updates */
    ...
    DTCContext->DtcEffects = DEM_CFG_TRIGGER_NONE;                                                                               /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
#else
    if (Dem_Memory_TestEventStorageTriggerIsFulfilled(DTCContext->EventId, DTCContext->DtcEffects) == TRUE)
  ...
  { /* store OBD Freeze Frame depending on DTC triggers, no event entry required */
    Dem_Memory_TriggerObdFreezeFrameStorage(DTCContext);                                                                       /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
...
)
{
  ...
  {
    if (Dem_Memory_TestEventStorageTriggerIsOnFdc(EventId) == TRUE)
    {
...
# if (DEM_CFG_SUPPORT_USER_CONTROLLED_WIR_DURING_CDTCS == STD_OFF)
  if (Dem_Conditions_EnableCondition_TestFulfilled(DEM_ENABLECONDITION_CONTROLDTCSETTING) == TRUE)
# endif
  {
  ...
  )
{
  ...
  if (((Dem_UDSStatus_Test_TF(lDtcStatus) == TRUE) && (Dem_UDSStatus_Test_CDTC(lDtcStatus) == TRUE))
    || ((Dem_Cfg_IsObdSupportedInVariant() == TRUE) && (Dem_Core_Event_TestMilAssigned(EventId) == TRUE) && (Dem_UDSStatus_Test_WIR(lDtcStatus) == TRUE))
     )
  {
    lIsActive = TRUE;
  }
  ...
  )
{
  ...
  if ((Dem_UDSStatus_Test_TF(lDtcStatus) == FALSE) && (Dem_UDSStatus_Test_CDTC(lDtcStatus) == TRUE)
    && ((Dem_Cfg_IsObdSupportedInVariant() == FALSE) || ((Dem_Core_Event_TestMilAssigned(EventId) == FALSE) || (Dem_UDSStatus_Test_WIR(lDtcStatus) == FALSE))))
  {
