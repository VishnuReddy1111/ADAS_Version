...
{
  if ((EventContext->EventId >= Dem_MemState_MemoryGetFirstEvent(DEM_CFG_MEMORYID_PRIMARY))
      && (EventContext->EventId <= Dem_MemState_MemoryGetLastEvent(DEM_CFG_MEMORYID_PRIMARY)))
  {
  ...
  )
{
  ...
  uint16 lLastFailedCycle = Dem_MemoryEntry_GetCyclesSinceLastFailed(DidCopyInfoPtr->MemoryEntryId);
  ...
  )
{
  ...
  uint16 lLastFailedWarmUpCycle = Dem_MemoryEntry_GetWarmUpCyclesSinceLastFailed(DidCopyInfoPtr->MemoryEntryId);
  ...
  )
{
  ...
  uint16 lFirstFailedCycle = Dem_MemoryEntry_GetCyclesSinceFirstFailed(DidCopyInfoPtr->MemoryEntryId);
