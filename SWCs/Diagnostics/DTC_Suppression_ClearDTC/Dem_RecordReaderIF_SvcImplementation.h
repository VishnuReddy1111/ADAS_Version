...
# if (DEM_CFG_SUPPORT_OBDII == STD_ON)
    if (Dem_Cfg_IsObdIIExclusivelySupportedInVariant() == TRUE)
    {
      ...
      do
      {
        ...
        if ((Dem_Cfg_ObdFFSlotCountInVariant() != lSelectedIndex)
...
Dem_DcmReadDataOfOBDFreezeFrame(
  ...
  uint8  DataElementIndexOfPID,
  P2VAR(uint8, AUTOMATIC, DEM_DCM_DATA)  DestBuffer,                                                                             /* PRQA S 3206, 3673 */ /* MD_DEM_3206, MD_DEM_8.13 */
  P2VAR(uint8, AUTOMATIC, DEM_DCM_DATA)  BufSize                                                                                 /* PRQA S 3206, 3673 */ /* MD_DEM_3206, MD_DEM_8.13 */
  )
{
  Std_ReturnType lReturnValue;
  Dem_Det_ErrorRegisterDefine()
...
)
{
  ...
  if (Dem_APIChecks_IsDTCFormatUDS(DTCFormat) == FALSE)
  {
    Dem_Det_ErrorRegisterSet(DEM_E_PARAM_DATA)
  }
  else
...
# endif
  ...
  {
    /* ----- Implementation ------------------------------------------------ */
    lReturnValue = Dem_Client_ClientAccess_GetNextJ1939FreezeFrame(ClientId, J1939DTC, OccurrenceCounter, DestBuffer, BufSize);  /* SBSW_DEM_POINTER_FORWARD_API_ARGUMENT_BUFFER_AND_SIZE */
  }
