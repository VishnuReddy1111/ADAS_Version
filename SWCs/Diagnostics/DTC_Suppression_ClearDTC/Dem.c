...
/* module specific MISRA deviations:
  ...
  MD_DEM_2897:
   ...
   MD_DEM_2998:
     ...
     Reason:      In specific configurations, the conditional operator is also 'True'.
     Risk:        None.
     Prevention:  None.
...
\ID SBSW_DEM_POINTER_MEMCPY_TIMESERIESENTRY
  \DESCRIPTION    In an internal function, a memory area is copied through a pointer to a memory entry. The entry
                  pointer is initialized by Dem_Memory_TimeSeries_GetEntryPtrSafe which will always return a pointer to a valid memory
...
\ID SBSW_DEM_POINTER_FORWARD_READOUTBUFFER_OBDFREEZEFRAME
  \DESCRIPTION    A pointer to OBD FreezeFrame buffer and buffer size are forwarded to another function. The function writes to the buffer,
                  or forwards the buffer and buffer size to another function which will write data to the buffer.
                  The memory entry pointer is initialized by Dem_MemObdFreezeFrame_FreezeFrameObdIIGetDataPtr() or 
                  Dem_FaultMemory_MemObdFreezeFrame_FreezeFrameObdOnUdsGetDataPtr() which will always return a pointer
...
\ID SBSW_DEM_WRITE_PIDF501_BUFFER
  \DESCRIPTION    An uint8 value is written to a PIDF501 data block array, which is checked to be greated than DEM_READINESS_DIDF501_SIZE in 
                  Dem_GlobalDiagnostics_Readiness_ReadDataOfDIDF501() and forwarded to subfunctions.
...
\ID SBSW_DEM_POINTER_OBD_FREEZEFRAME_BUFFER
  \DESCRIPTION    A destination buffer pointer and buffer size is passed into a function that will write to the passed
                  memory area. The pointer argument is returned from Dem_MemObdFreezeFrame_FreezeFrameObdIIGetDataPtr() or 
                  Dem_FaultMemory_MemObdFreezeFrame_FreezeFrameObdOnUdsGetDataPtr() which will always
  ...
  \COUNTERMEASURE \R [CM_DEM_R_MEMORY_GET_OBDDATAPTR]
                  \R [CM_DEM_R_MEMORY_GET_OBDDATAPTR_OBDONUDS]
...
\ID SBSW_DEM_POINTER_PRESTORAGE_OBD_FREEZEFRAME_BUFFER
  \DESCRIPTION    A destination buffer pointer and buffer size is passed into a function that will write to the passed
                  memory area. The pointer argument is returned from Dem_Prestore_PrestorageEntryGetObdFFDataPtr()
                  which will always return a valid pointer. The corresponding buffer size is received with function
...
\ID SBSW_DEM_POINTER_WRITE_TIMESERIESENTRY
  \DESCRIPTION    In an internal function, data is written through a pointer to a time series entry, or the pointer is
                  ...
                  The memory entry pointer is initialized by Dem_Memory_TimeSeries_GetEntryPtrSafe which will always return a pointer
 ...
 \ID SBSW_DEM_POINTER_READ_TIMESERIESENTRY
  \DESCRIPTION    A pointer to a TimeSeriesEntry structure is passed to a function expecting a valid
                  pointer. The pointer of the TimeSeriesEntry is initialized by Dem_Memory_TimeSeries_GetEntryPtrSafe and thus 
                  it is always valid.(see also SBSW_DEM_POINTER_WRITE_TIMESERIESENTRY).
  \COUNTERMEASURE \R [CM_DEM_R_TIMESERIES_GET_ENTRY]
                  \M [CM_DEM_M_TIMESERIESINDEX_LIMITS]
                  \M [CM_DEM_M_SIZEOF_MEMORYDATAPTR]
                  \S [CM_DEM_S_TIMESERIESINDEX_TYPE]
...
\ID SBSW_DEM_ARRAY_WRITE_MEMORYCHRONOLOGY
  \DESCRIPTION    In a function, data is written to array Dem_Cfg_PrimaryChronology[], Dem_Cfg_UserDefinedXYZChronology[] or
                  Dem_Cfg_TimeSeriesChronology[] using a function argument as index.
...
\ID SBSW_DEM_ARRAY_WRITE_QUALIFIEDACTIVEEVENTCOUNT
  \DESCRIPTION    In a function, data is written to array Dem_ActivationMode_QualifiedActiveEventCount[] using function arguments as indices.
                  The array Dem_ActivationMode_QualifiedActiveEventCount[] has constant size.
  \COUNTERMEASURE \R [CM_DEM_R_QUALIFIEDACTIVEEVENTCOUNT_INDEX_CHECK]

\ID SBSW_DEM_ARRAY_WRITE_DTCCLASSMILCOUNT
  \DESCRIPTION    In a function, data is written to array Dem_ActivationMode_DtcClassMILCount[] using function arguments as indices.
                  The array Dem_ActivationMode_DtcClassMILCount[] has constant size.
...
\CM CM_DEM_R_TIMESERIES_GET_ENTRY
  Dem_Memory_TimeSeries_GetEntryPtrSafe will always return a valid time series entry even when the passed memory index is not a handle to a time series entry.
...
\CM CM_DEM_R_PRESTORAGE_GET_OBDFFPTR
  Dem_Prestore_PrestorageEntryGetObdFFDataPtr will always return a valid pointer even when the passed prestorage index is not valid.
...
\CM CM_DEM_R_MEMORY_GET_OBDDATAPTR
  Dem_MemObdFreezeFrame_FreezeFrameObdIIGetDataPtr will always return a valid pointer even when the passed record index is not valid.

  \CM CM_DEM_R_MEMORY_GET_OBDDATAPTR_OBDONUDS
  Dem_FaultMemory_MemObdFreezeFrame_FreezeFrameObdOnUdsGetDataPtr will always return a valid pointer even when the passed record index is not valid.
...
\CM_DEM_R_CHECK_WRITE_VARIABLE_SIZE
...
\CM CM_DEM_R_CHRONOLOGY_INDEX_CHECK
  ...
  Dem_Cfg_UserDefinedXYZChronology[] or Dem_Cfg_TimeSeriesChronology[].
...
\CM CM_DEM_R_QUALIFIEDACTIVEEVENTCOUNT_INDEX_CHECK
  Verify that all array indices are in range before writing to Dem_ActivationMode_QualifiedActiveEventCount[][] .

\CM CM_DEM_R_DTCCLASSMILCOUNT_INDEX_CHECK
  Verify that all array indices are in range before writing to Dem_ActivationMode_DtcClassMILCount[][] .
...
\CM CM_DEM_S_TIMESERIESINDEX_TYPE
  [SMI-2200] If time series snapshots are configured (also known as Multi Sample Snapshot Record) i.e. if any of 
  the preprocessor switches DEM_CFG_SUPPORT_TIME_SERIES_<PAST/FUTURE>_<NORMAL/FAST> are defined as STD_ON:
  The user of MICROSAR Safe shall ensure that table Dem_Cfg_MemoryDataPtr[] contains a pointer of type 
   Dem_Cfg_TimeSeriesEntryType for the indices DEM_CFG_MEMORY_TIME_SERIES_INDEX (inclusive) through 
   DEM_CFG_MEMORY_TIME_SERIES_INDEX + DEM_CFG_GLOBAL_TIMESERIES_SNAPSHOTS_SIZE (exclusive). 
  The user of MICROSAR Safe shall also ensure that in table Dem_Cfg_MemoryInfoTable, in the row indicated 
   by preprocessor macro DEM_CFG_MEMORYID_TIMESERIES, the member FirstMemoryIndexOfMemoryInfoTable is 
   equal to DEM_CFG_MEMORY_TIME_SERIES_INDEX and the member MaxSizeOfMemoryInfoTable is equal to 
   DEM_CFG_GLOBAL_TIMESERIES_SNAPSHOTS_SIZE. 
  The table Dem_Cfg_MemoryDataPtr[], Dem_Cfg_MemoryInfoTable[] are generated into Dem_Lcfg.c. 
  The macros DEM_CFG_MEMORY_TIME_SERIES_INDEX and DEM_CFG_GLOBAL_TIMESERIES_SNAPSHOTS_SIZE are generated into Dem_Lcfg.h. 
  The type Dem_Cfg_TimeSeriesEntryType is generated into Dem_Lcfg.h. 
  The preprocessor macro DEM_CFG_MEMORYID_TIMESERIES is generated into Dem_Lcfg.h. 
...
\CM CM_DEM_S_CHECK_CHRONOLOGY_MEMORYINFOTABLE
 ...
 The user of MICROSAR Safe shall also verify that size of each of the aforementioned Chronology array, is equal to the 
