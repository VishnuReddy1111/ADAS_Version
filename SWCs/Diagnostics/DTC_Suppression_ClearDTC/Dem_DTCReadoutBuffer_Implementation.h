...
# if(DEM_CFG_SUPPORT_TIME_SERIES_FUTURE_NORMAL == STD_ON)
    Dem_DTCReadoutBuffer_GetReadoutBufferSafe(ReadoutBufferId)->UnusedFutureSamples_NormalRate =
      Dem_Memory_TimeSeries_EntryManager_GetUnusedFutureSamplesNormal(Dem_Memory_TimeSeries_GetEntryPtrSafe(MemoryIndex));          /* SBSW_DEM_ARRAY_POINTER_READOUTBUFFER */ /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
# endif
# if(DEM_CFG_SUPPORT_TIME_SERIES_FUTURE_FAST == STD_ON)
    Dem_DTCReadoutBuffer_GetReadoutBufferSafe(ReadoutBufferId)->UnusedFutureSamples_FastRate =
      Dem_Memory_TimeSeries_EntryManager_GetUnusedFutureSamplesFast(Dem_Memory_TimeSeries_GetEntryPtrSafe(MemoryIndex));            /* SBSW_DEM_ARRAY_POINTER_READOUTBUFFER */ /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
# endif
  }
#endif
}
...
# if ((DEM_CFG_SUPPORT_OBDII == STD_ON) && (DEM_CFG_SUPPORT_OBDII_FREEZEFRAME_IN_SVC19 == STD_ON))
  ...
  if (lSelectedIndex < Dem_Cfg_ObdFFSlotCountInVariant())
  {
    ...
    lSourceEntry = Dem_MemObdFreezeFrame_FreezeFrameObdIIGetDataPtr(lSelectedIndex);
...
# if (DEM_CFG_SUPPORT_OBDONUDS == STD_ON) 
  ...
  if (lSelectedIndex < Dem_Cfg_ObdFFSlotCountInVariant())
  {
    ...
    lSourceEntry = Dem_FaultMemory_MemObdFreezeFrame_FreezeFrameObdOnUdsGetDataPtr(lSelectedIndex, Dem_RecordReader_0x1904_OBDonUDSFF_0x00);
    ...
    lSourceEntry = Dem_FaultMemory_MemObdFreezeFrame_FreezeFrameObdOnUdsGetDataPtr(lSelectedIndex, Dem_RecordReader_0x1904_OBDonUDSFF_0xF0);
...
# if (DEM_FEATURE_NEED_COUNTER_READOUT_BUFFER == STD_ON)
  if (ReadoutBufferId < DEM_MEMORYENTRY_HANDLE_INVALID)
  {
