 ...
 ****************************************************************************/
/*!
 * \brief         Evaluates primary/user defined memory entry to restore parts of
  ...
  )
{
  ...
  if (lEventId != DEM_EVENT_INVALID)
  {
    if ((lEventId >= Dem_Cfg_GlobalEventCount())                                                                                 
      || (Dem_Cfg_EventDestination(lEventId) != TranslatedMemoryId)
#if (DEM_CFG_SUPPORT_TYPE1_COMBINATION == STD_ON)
  ...
  )
{
  ...
  if (lEventId != DEM_EVENT_INVALID)
  {
    ...
    { 
      ...
      lOtherIndex = Dem_MemAccess_FindMemoryIndex_StoredEvent(TranslatedMemoryId, lEventId);
      ...
      {
        Dem_MemAccess_MemoryFreeIndex(TranslatedMemoryId, lOtherIndex);
...
# if (DEM_CFG_STORAGE_AT_FAILED == STD_ON)
  if (Dem_Memory_TestEventStorageTriggerIsOnFailed(EventId) == TRUE)
  {
...
# if (DEM_CFG_STORAGE_AT_PENDING == STD_ON)
  if (Dem_Memory_TestEventStorageTriggerIsOnPending(EventId) == TRUE)
  {
...
#if (DEM_CFG_SUPPORT_TYPE1_COMBINATION == STD_ON)
  if ((Dem_Cfg_EventIsCombined(EventId) == TRUE) && (Dem_Memory_TestEventStorageTriggerIsOnFdc(EventId) == FALSE))
  {
