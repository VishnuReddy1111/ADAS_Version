  ...
  )
{
  ...
  if (Dem_FaultMemory_Memory_IsEventDestination(TranslatedMemoryId) == TRUE)
  {
    ...
    { /* report by event id, set first event index */
      lFilterDataPtr->Iterator.EventIndex = Dem_MemState_MemoryGetFirstEvent(TranslatedMemoryId);                                /* SBSW_DEM_ARRAY_POINTER_FILTERDATA */
...
# if (DEM_FEATURE_NEED_REPORT_CHRONOLOGICALLY == STD_ON)
  ...
  if (lStoredChronoIter == DEM_FILTERDATA_FILTER_INIT)
  {
    ...
    Dem_Memory_AllocMemEntriesChrono_IterInit(lTranslatedMemoryId, &lStoredEventsIter);                                          /* SBSW_DEM_CALL_ITERATOR_POINTER */
  }
  else
  {
    /* last stored index loaded, set and increment it*/
    Dem_Memory_AllocMemEntriesChrono_IterResume(lTranslatedMemoryId, lStoredChronoIter, &lStoredEventsIter);                     /* SBSW_DEM_CALL_ITERATOR_POINTER */
  }
...
# endif        
  ...
  {
    lFilterDataPtr->Iterator.EventIndex = Dem_Memory_GetFirstEvent(lTranslatedMemoryId);                                         /* SBSW_DEM_ARRAY_POINTER_FILTERDATA */
    lFilterDataPtr->FilterDtcStrategy = Dem_FilterData_FilterEventBased;                                                         /* SBSW_DEM_ARRAY_POINTER_FILTERDATA */
  ...
  )
{
  ...
  if (Dem_Memory_GetFirstEvent(lTranslatedMemoryId) != DEM_EVENT_INVALID)
  {
    /* at least one event is available which may match the filter criteria */
    for (lEventId = Dem_Memory_GetFirstEvent(lTranslatedMemoryId);
         lEventId <= Dem_Memory_GetLastEvent(lTranslatedMemoryId);
          ++lEventId)
    {
      if (Dem_Client_FilterData_FilterForEventMatch(FilterId, lEventId) == E_OK)
      {
  ...
  )
{
  ...
  if (Dem_Cfg_IsDtcFormatSupportedInVariant(DTCFormat) == TRUE)
  {
  ...
  )
{
  ...
  lFilterDataPtr->Iterator.EventIndex = Dem_MemState_MemoryGetFirstEvent(DEM_CFG_MEMORYID_PRIMARY);                              /* SBSW_DEM_ARRAY_POINTER_FILTERDATA */
  lFilterDataPtr->NodeIndex = NodeIndex;                                                                                               /* SBSW_DEM_ARRAY_POINTER_FILTERDATA */
  ...
  )
{
  ...
  if (lEventId != DEM_EVENT_INVALID)
  {
    ...
    lEventIdLast = Dem_MemState_MemoryGetLastEvent(DEM_CFG_MEMORYID_PRIMARY);
    /* iterate through the EventIds and check filter criteria */
...
# endif
      ...
      /*Get lamp status*/
      lReturnValue = Dem_Client_ClientAccess_CalculateEventJ1939LampStatus(lEventId, lFilterDataPtr->NodeIndex, FilterResult);                /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */ 
      if (lReturnValue == E_OK)
      {
        /* Get OCC and DTC */
  ...
  )
{
  ...
  Dem_GlobalDiagnostics_Readiness_ReadDataOfDM05(&lDestinationBuffer);                                                /* SBSW_DEM_POINTER_LOCAL_DESTINATIONBUFFER */
