...
/* ****************************************************************************
 % Dem_Core_Event_IsEventAvailabilitySupported
 *****************************************************************************/
/*!
...
{
  return (boolean)(DEM_CFG_SUPPORT_EVENTAVAILABLE == STD_ON);
}
  ...
  )
{
  ...
  return Dem_Cfg_GetStatusData().ExtendedEventStatus[EventId];                                                                   /* PRQA S 2843 */ /* MD_DEM_Dir4.1_ReadOperation */
#else
...
/* ****************************************************************************
 % Dem_Core_Event_GetEventReadinessGroup
 *****************************************************************************/
 /*!
  * Internal comment removed.
 *
 *
  */
DEM_LOCAL_INLINE FUNC(uint8, DEM_CODE)
Dem_Core_Event_GetEventReadinessGroup(                                                                                                     /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  CONST(Dem_EventIdType, AUTOMATIC) EventId                                                                                      /* PRQA S 3206 */ /* MD_DEM_3206 */
)
{
  uint8 lReturn;
...
/* ****************************************************************************
 % Dem_Core_Event_TestHighPriorReadinessGroup
 *****************************************************************************/
 /*!
  * Internal comment removed.
 *
...
Dem_Core_Event_TestHighPriorReadinessGroup(                                                                                           /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  Dem_EventIdType EventId
)
{
...
# endif
        ...
        Dem_Readiness_Completion_TriggerCompletionRecalculation(Dem_Core_Event_GetEventReadinessGroup(EventId));
...
)
{
  ...
  EventContext->QualifyStatus = Dem_Event_GetQualifyStatus(EventId);                                                          /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
...
)
{
  ...
  if ((Dem_DTC_IsAgingForAllDTCsEnabled() == TRUE) || (Dem_Memory_TestEventStorageTriggerIsOnConfirmed(EventId) == FALSE))
  {
    lReturnValue = Dem_UDSStatus_Reset_TF(lReturnValue);
  }

  if (Dem_Memory_TestEventStorageTriggerIsOnConfirmed(EventId) == FALSE)
  {
...
/* <<<< -------------------------------- Leave Critical Section: DiagMonitor */
    ...
    if (lNewDisconnectedStatus == FALSE)
    {
      Dem_Readiness_Completion_UpdateCompletionStatus(Dem_Core_Event_GetEventReadinessGroup(EventId), FALSE);
    }
  ...
  )
{
  ...
  if ( (Dem_Cfg_EventUdsDtc(EventId) != DEM_CFG_DTC_UDS_INVALID)
    ...
    && (Dem_Conditions_StorageCondition_TestEventStorageConditionsFulfilled(EventId) == TRUE) )                                             
  {
  ...
  )
{
  if (Dem_Event_TestLatchedTestFailed(EventId) == FALSE)
  {
...
#endif
    {
      ...
      Dem_DiagnosticObserver_ProcessEventPassed(&lEventContext, &lDTCContext);                                                    /* SBSW_DEM_POINTER_FORWARD_STACK */
...
)
{
  ...
  if ( (Dem_EventInternalStatus_Test_QualifyStatus_CDTC(lEventContext.QualifyStatus) == FALSE)
    && (Dem_UDSStatus_Test_CDTC(lEventContext.OldEventStatus) == TRUE)
    )
  { /* set confirmed status to qualified */
    lEventContext.QualifyStatus = Dem_EventInternalStatus_Set_QualifyStatus_CDTC(lEventContext.QualifyStatus);
    Dem_Event_SetQualifyStatus(EventId, lEventContext.QualifyStatus);
...
# endif
  ...
  if ( (Dem_EventInternalStatus_Test_QualifyStatus_WIR(lEventContext.QualifyStatus) == FALSE)
    && (Dem_UDSStatus_Test_WIR(lEventContext.OldEventStatus) == TRUE))
  { /* set warningIndicatorRequested status to qualified */
    lEventContext.QualifyStatus = Dem_EventInternalStatus_Set_QualifyStatus_WIR(lEventContext.QualifyStatus);
    Dem_Event_SetQualifyStatus(EventId, lEventContext.QualifyStatus);
...
# endif
  ...
  Dem_Notifications_EventStatusChanged(EventId,
                                       lEventContext.OldExternalEventStatus,
                                       Dem_Event_GetExternalUDSStatus(EventId, lEventContext.OldEventStatus));
}                                                                                                                                /* PRQA S 6050 */ /* MD_MSR_STCAL */
...
/* ****************************************************************************
 % Dem_Core_Event_GetEnableConditionGroupId
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
/* ****************************************************************************
 % Dem_Core_Event_GetStorageConditionGroupId
 *****************************************************************************/
 /*!
 * Internal comment removed.
 *
...
/* ****************************************************************************
 % Dem_Core_Event_TestMilAssigned
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
...
{
  boolean lMilAssignedForEvent;
  Dem_Cfg_ComplexIterType lIndicatorIter;

  lMilAssignedForEvent = FALSE;
...
# if (DEM_CFG_PROCESS_CDTC_ALL_DTC == STD_OFF)
  ...
  lStoredStatus = Dem_DTCInternalStatus_GetStoredStatus(Dem_DTC_GetInternalStatus(EventContext->EventId));
...
# endif
  {
    ...
    lEventQualifyStatus = EventContext->QualifyStatus;
...
# else
    {
      ...
      lMemoryIndex = Dem_Memory_FindMemoryIndex_StoredEvent(EventContext->EventId);
      lMemoryEntryId = Dem_MemoryEntry_GetId(lMemoryIndex);
...
{
  uint16 lTranslatedMemoryId = Dem_Cfg_EventDestination(EventId);
  return (boolean)Dem_Memory_UserDefinedMemory_IsSRecNumerationCalculated(Dem_Memory_GetSRecNumerationType(lTranslatedMemoryId));
}
...
{
  uint16 lTranslatedMemoryId = Dem_Cfg_EventDestination(EventId);
  return (boolean)Dem_Memory_UserDefinedMemory_IsSRecNumerationCalculatedFiFo(Dem_Memory_GetSRecNumerationType(lTranslatedMemoryId));
}
...
{
  uint16 lTranslatedMemoryId = Dem_Cfg_EventDestination(EventId);
  return (boolean)Dem_Memory_UserDefinedMemory_IsSRecNumerationConfigured(Dem_Memory_GetSRecNumerationType(lTranslatedMemoryId));
}
  ...
  )
{
  ...
  {
    lReturnValue = Dem_Cfg_IsEventConsiderPtoStatusOfEventTable(EventId);
  }
