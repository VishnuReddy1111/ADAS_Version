      ...
      || (Dem_GetHiByte(DataId) == 0x00U))
    {
      ...
      do
      {
        ...
        { /* Event has a stored OBD freeze frame */
          ...
          {
            lSourceBuffer = (Dem_ConstSharedDataPtrType)Dem_MemObdFreezeFrame_FreezeFrameObdIIGetDataPtr(lSelectedIndex);
...
)
{
  ...
  if (Dem_Core_DTC_IsObdRelated(EventId) == TRUE)
  {
    ...
    do
    {
      ...
      {
        lActiveEventMemory = (boolean)(Dem_MemoryEntry_TestAgingOnlyEntry(Dem_MemoryEntry_GetId(lMemoryIndex)) == FALSE);
      }

      if (lActiveEventMemory == TRUE)
      {
        uint8 lUpdateState;
        Dem_Cfg_MemoryEntryHandleType lMemoryEntry = Dem_MemoryEntry_GetId(lMemoryIndex);
        ...
        { 
          ...
          Dem_ConstSharedDataPtrType lSourceBuffer = (Dem_ConstSharedDataPtrType)Dem_MemoryEntry_GetWwhObdFreezeFrameDataPtr(lMemoryEntry);    
  ...
  if (Dem_Core_DTC_IsObdRelated(EventId) == TRUE)
  {
    ...
    do
    {
      ...
      if (lSelectedIndex < Dem_Cfg_ObdFFSlotCountInVariant())
      {
        ...
        if (Dem_Event_TestDataUpdateInProgress(lUpdateState) == FALSE)
        {
          lSourceEntry = Dem_FaultMemory_MemObdFreezeFrame_FreezeFrameObdOnUdsGetDataPtr(lSelectedIndex, lFreezeFrameId);
  ...
  do
  {
    ...
    { /* The record currently exists, but without critical section it could age or displace */
      ...
      { /* This call does not currently interrupt a concurrent data update */
        ...
        lEntry = Dem_Memory_TimeSeries_GetEntryPtrSafe(lTimeSeriesMemoryIndex);

        if (Dem_Memory_TimeSeries_TestSRecIsStored(lEntry, RecordNumber) == TRUE)                                                /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
        {
          Dem_ConstSharedDataPtrType lSourceBuffer;
          Dem_Cfg_MemoryIndexType lMemoryIndex;

          if (Dem_Memory_TimeSeries_TestRecordNumberIsNormalRate(EventId, RecordNumber) == TRUE)
          {
            lSourceBuffer = 
              Dem_Memory_TimeSeries_EntryManager_GetNormalConstDataPtr(lEntry,                                                   /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
              RecordNumber - Dem_Memory_TimeSeries_GetFirstNormalRateRecordNumber(EventId)
              ); 
          }
          else /* RecordNumber represents fast rate time series snapshot */
          {
            lSourceBuffer = 
              Dem_Memory_TimeSeries_EntryManager_GetFastConstDataPtr(lEntry,                                                     /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
    ...
    do
    {
      ...
      { 
        ...
        { 
          ...
          {
            Dem_SnapshotEntry_IteratorFindLastStoredEntry(EventId, lMemoryEntry, &lSnapshotEntryIter);                           /* SBSW_DEM_POINTER_FORWARD_STACK */
          }
          else
          {
            Dem_SnapshotEntry_IteratorFindEntryWithRecordNumber(EventId, RecordNumber, lMemoryEntry , &lSnapshotEntryIter);      /* SBSW_DEM_POINTER_FORWARD_STACK */
          }
    ...
    { /* A stored record needs to be retrieved from event memory */
      ...
      do
      {
        ...
        { /* The record currently exists, but without critical section it could age or displace */
          ...
          if (Dem_Event_TestDataUpdateInProgress(lUpdateState) == FALSE)
          { /* This call does not currently interrupt a concurrent data update */
            lMemoryEntryId = ((lRecordType != DEM_CFG_EREC_TYPE_GLOBAL) ? Dem_MemoryEntry_GetId(lMemoryIndex) : DEM_MEMORYENTRY_HANDLE_INVALID); /* PRQA S 2998 */ /* MD_DEM_2998 */ 
            ...
            { /* The DTC has an event entry, but the requested record was not yet stored */
              ...
              && (Dem_RecordReader_ERec_IsInternalERecReadable(lCfgERecIndex, lMemoryEntryId) == FALSE))                         /* PRQA S 3415 */ /* MD_DEM_13.5_cf */
            {
              Dem_Data_ResetWriteIndexDestinationBuffer(DestinationBuffer);                                                      /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
              lReturnValue = DEM_NO_SUCH_ELEMENT;
            }
            else
            {
              ...
              else
              {
                ...
                Dem_DataReport_InitDidCopyInfo(&lDidCopyInfo, DestinationBuffer, EventId, lMemoryEntryId, DEM_MEMORYENTRY_HANDLE_INVALID); /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
...
# if (DEM_CFG_SUPPORT_TIME_SERIES_POWERTRAIN == STD_OFF)
    else if (Dem_Memory_TimeSeries_TestRecordNumberIsValid(EventId, RecordNumber) == TRUE)
    {
      Dem_EventIdType lMemoryRepresentativeEventId;
