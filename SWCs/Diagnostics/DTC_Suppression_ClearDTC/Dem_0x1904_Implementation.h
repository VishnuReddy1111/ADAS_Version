...
#  if (DEM_CFG_SUPPORT_TIME_SERIES_POWERTRAIN == STD_OFF)
  if (Dem_Memory_TimeSeries_TestRecordNumberIsFastRate(lEventId, RecordNumber) == TRUE)
  {
    lSource = Dem_DTCReadoutBuffer_SnapshotSource_TimeSeries_Fast;
  }
  else if (Dem_Memory_TimeSeries_TestRecordNumberIsNormalRate(lEventId, RecordNumber) == TRUE)
  {
...
#  if (DEM_FEATURE_NEED_TIME_SERIES == STD_ON) && (DEM_CFG_SUPPORT_TIME_SERIES_POWERTRAIN == STD_OFF)
  if (Dem_Memory_TimeSeries_TestRecordNumberIsValid(lEventId, lRecordNumber) == TRUE)
  {
  ...
  )
{
  ...
  switch (Dem_DTCReadoutBuffer_GetSnapshotRecordSource(ReadoutBufferId))
  {
    ...
    case Dem_DTCReadoutBuffer_SnapshotSource_TimeSeries_Fast:
    {
      ...
      if (Dem_Memory_TimeSeries_TestEventSupportFastRate(lEventId) == TRUE)
      {
        Dem_Cfg_ComplexIterInit(&lSourceIterator,                                                                                /* SBSW_DEM_CALL_ITERATOR_POINTER */
          Dem_Memory_TimeSeries_GetFirstFastRateRecordNumber(lEventId),
          (uint8)(Dem_Memory_TimeSeries_GetLastFastRateRecordNumber(lEventId) + 1U));
      }
      else
      {
        Dem_Cfg_ComplexIterInit(&lSourceIterator, 0, 0);                                                                         /* SBSW_DEM_CALL_ITERATOR_POINTER */
    ...
    case Dem_DTCReadoutBuffer_SnapshotSource_TimeSeries_Normal:
    {
      ...
      if (Dem_Memory_TimeSeries_TestEventSupportNormalRate(lEventId) == TRUE)
      {
        Dem_Cfg_ComplexIterInit(&lSourceIterator,                                                                                /* SBSW_DEM_CALL_ITERATOR_POINTER */
          Dem_Memory_TimeSeries_GetFirstNormalRateRecordNumber(lEventId),
          (uint8)(Dem_Memory_TimeSeries_GetLastNormalRateRecordNumber(lEventId) + 1U));
      }
      else
      {
        Dem_Cfg_ComplexIterInit(&lSourceIterator, 0, 0);                                                                         /* SBSW_DEM_CALL_ITERATOR_POINTER */
...
# else
        if(Dem_Memory_TimeSeries_GetFirstFastRateRecordNumber(lEventId) < Dem_Memory_TimeSeries_GetFirstNormalRateRecordNumber(lEventId))
        {
    ...
    if (Dem_RecordReader_0x1904_TestRecordMatch(ReadoutBufferId, lRecordNumber) == TRUE)
    {
      ...
      else
      {
        ...
        if (Dem_RecordReader_0x1904_TimeSeriesSRecIsReadableInVirtualBuffer(ReadoutBufferId, lRecordNumber) == TRUE)
        {
          ...
          if (Dem_Memory_TimeSeries_TestRecordNumberIsFastRate(lEventId, lRecordNumber) == TRUE)
          {
            lpSourceBuffer = 
              Dem_Memory_TimeSeries_EntryManager_GetFastConstDataPtr(lTimeSeriesEntry,
                                                    lRecordNumber - Dem_Memory_TimeSeries_GetFirstFastRateRecordNumber(lEventId));     /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
          }
          else /* lRecordNumber represents normal rate time series snapshot */
          {
            lpSourceBuffer =
              Dem_Memory_TimeSeries_EntryManager_GetNormalConstDataPtr(lTimeSeriesEntry,
                                                   lRecordNumber - Dem_Memory_TimeSeries_GetFirstNormalRateRecordNumber(lEventId));    /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */
          }
...
)
{
  ...
  if (Dem_Core_Event_HasConfiguredOrCalculatedSRecs(EventId) == TRUE)
  {
    ...
    *SizeOfFreezeFrame = (uint16)(Dem_SnapshotEntry_GetNumberOfStoredSnapshotRecords(lReadoutBufferMemoryEntryId)                /* SBSW_DEM_POINTER_WRITE_ARGUMENT */
  ...
  )
{
  ...
  if (((boolean)(Dem_Memory_TimeSeries_TestEventSupportNormalRate(EventId))) == TRUE)
  {
...
)
{
  ...
  else
  {
    ...
    Dem_Mem_ConstTimeSeriesEntryPtrType lTimeSeriesEntry = Dem_Memory_TimeSeries_GetEntryPtrSafe(Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId));
    Dem_EventIdType lEventId = Dem_DTCReadoutBuffer_GetEventId(ReadoutBufferId, DEM_DTCREADOUTBUFFER_DATAINDEX_DEFAULT);

    if (Dem_Memory_TimeSeries_TestRecordNumberIsNormalRate(lEventId, RecordNumber) == TRUE)
    {
      lSRecStoredIndex = RecordNumber - Dem_Memory_TimeSeries_GetFirstNormalRateRecordNumber(lEventId);
    ...
    else if (Dem_Memory_TimeSeries_TestRecordNumberIsFastRate(lEventId, RecordNumber) == TRUE)
    {
      lSRecStoredIndex = RecordNumber - Dem_Memory_TimeSeries_GetFirstFastRateRecordNumber(lEventId);
