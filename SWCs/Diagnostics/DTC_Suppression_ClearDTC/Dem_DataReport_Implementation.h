...
)
{
  ...
  lTimeSeriesEntryPtr = Dem_Memory_TimeSeries_GetEntryPtrSafe(Dem_DTCReadoutBuffer_GetTimeSeriesEntryIndex(ReadoutBufferId));
#endif

  /* Get meta information about the time series record */
  Dem_DataReport_GetTimeSeriesPowertrainSequentialInfo(lTimeSeriesEntryPtr, lEventId, &lTimeSeriesInfo);                         /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */ /* SBSW_DEM_POINTER_FORWARD_STACK */
  ...
  Dem_DataReport_WriteTimeSeriesPowertrainSequentialHeader(&lSourceHeaderBuffer,                                                 /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */ /* SBSW_DEM_POINTER_FORWARD_STACK */
        ...
        Dem_Memory_TimeSeries_EntryManager_EventDidIterNext(&lDidIter))                                                          /* SBSW_DEM_CALL_ITERATOR_POINTER */
  {
    ...
    Dem_DataReport_WriteTimeSeriesPowertrainSequentialDid(DestinationBuffer,                                                     /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */ /* SBSW_DEM_POINTER_READ_TIMESERIESENTRY */  /* SBSW_DEM_POINTER_FORWARD_STACK */ 
...
#else
  DidCopyInfoPtr->ReadoutBufferId = DEM_MEMORYENTRY_HANDLE_INVALID;                                                              /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
...
)
{
  if (DestinationBuffer->BufferSize > DestinationBuffer->WriteIndex)
  {
    ...
    Dem_DataReport_InitDidCopyInfo(&lDidCopyInfo, DestinationBuffer, EventId , MemoryEntryId, ReadoutBufferId);                  /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
  ...
  )
{
  if (DestinationBuffer->BufferSize > DestinationBuffer->WriteIndex)
  {
    ...
    Dem_DataReport_InitDidCopyInfo(&lDidCopyInfo, DestinationBuffer, lEventId, MemoryEntryId, ReadoutBufferId);                  /* SBSW_DEM_CALL_DIDCOPYINFO_INIT */
    Dem_DataReport_InitDidSourceBufferReadInfo(&lDidSourceBufferReadInfo, SourceBuffer);                                         /* SBSW_DEM_CALL_DIDSOURCEBUFFERREADINFO_INIT */
...
{
  if (Dem_Data_GetRemainingSizeOfDestinationBuffer(DestinationBuffer) > 0u)                                                      /* SBSW_DEM_POINTER_FORWARD_ARGUMENT */
  {
    uint16 lDidCount;
    uint16 lDidCountWriteIndex;
    Dem_Cfg_EventObdDidIterType lDidIter;
      ...
      { /* look for the requested data element of the PID */
        ...
        if (DataElementIndex == lDataElementIndex)
        {
          ...
          if (lElementSize <= *BufSize)
          {
            ...
            lObdFreezeFrameDataPtr = Dem_MemObdFreezeFrame_FreezeFrameObdIIGetDataPtr(FreezeFrameIndex);
  ...
  )
{
  ...
  # endif
    {
      lPtrToSource = (Dem_ConstSharedDataPtrType)Dem_MemObdFreezeFrame_FreezeFrameObdIIGetDataPtr(BufferIdx.FreezeFrameIndex);
    }
  ...
  )
{
  ...
  Dem_DataReport_DidCopyInfoType lDidCopyInfo;
  Dem_DataReport_DidSourceBufferReadInfoType lDidSourceBufferReadInfo;
  Dem_EventIdType lEventId = Dem_MemoryEntry_GetEventId(MemoryEntry);
...
Dem_RecordReader_DataReport_CopyDidFromObdOnUdsFreezeFrame(
  Dem_EventIdType  EventId,
  uint16  DataId,
  /* Dem_Cfg_MemoryEntryHandleType  MemoryEntry, */
  ...
  )
{
  ...
  Dem_DataReport_DidCopyInfoType lDidCopyInfo;
  Dem_DataReport_DidSourceBufferReadInfoType lDidSourceBufferReadInfo;
  Dem_Cfg_MemoryEntryHandleType lMemoryEntryId = DEM_MEMORYENTRY_HANDLE_INVALID;
  Dem_EventIdType lEventId = DEM_EVENT_INVALID;

  if (MemoryIndex != DEM_MEM_INVALID_MEMORY_INDEX)
  {
    Dem_Cfg_MemoryEntryHandleType lEventMemoryEntryId = Dem_MemoryEntry_GetId(MemoryIndex);
    if (Dem_MemoryEntry_TestAgingOnlyEntry(lEventMemoryEntryId) == FALSE)
    {    
      lMemoryEntryId = lEventMemoryEntryId;
      lEventId = EventId;
    }
  }
