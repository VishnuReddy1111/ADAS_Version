...
Dem_Mem_FreezeFrameGetTimestamp(                                                                                                 /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  CONST(uint8, AUTOMATIC)  Index
  )
{
#if (DEM_CFG_SUPPORT_OBDII == STD_ON)
 ...
 */
DEM_LOCAL_INLINE FUNC(void, DEM_CODE)
Dem_Mem_FreezeFrameSetTimestamp(                                                                                                 /* PRQA S 3219 */ /* MD_DEM_14.1_ACCESSOR */
  CONST(uint8, AUTOMATIC)  Index,
  ...
  )
{
  ...
  lPermanentEntryEventId = Dem_Cfg_PermanentData.PermanentDataEntry[Index].EventId;
#else
  DEM_IGNORE_UNUSED_CONST_ARGUMENT(Index)                                                                                        /* PRQA S 3112 */ /* MD_DEM_14.2 */
#endif
...
/* ****************************************************************************
 % Dem_Mem_FindOBDFreezeFrameSlot
 *****************************************************************************/
/*!
 * Internal comment removed.
 *
  ...
  )
{
  ...
  if (( lSelectedIndex == Dem_Cfg_ObdFFSlotCountInVariant()) && (Dem_Cfg_IsObdIIExclusivelySupportedInVariant() == TRUE))
  {
  ...
  )
{
  ...
  {
#if(DEM_CFG_SUPPORT_PERMANENT == STD_ON) || (DEM_CFG_SUPPORT_OBDII_OR_OBDONUDS == STD_ON)
    Dem_EventIdType lEventId;
#endif
    ...
    if (Dem_Cfg_IsPermanentSupportedInVariant() == TRUE)
    {
      ...
      lPermanentIndex = Dem_MemState_MemoryGetMaxSize(DEM_CFG_MEMORYID_PERMANENT);
      while (lPermanentIndex != 0u)
      {
        ...
        if (lEventId != DEM_EVENT_INVALID)
        {
          Dem_MemState_MemorySetCurrentSize(DEM_CFG_MEMORYID_PERMANENT, Dem_MemState_MemoryGetCurrentSize(DEM_CFG_MEMORYID_PERMANENT) + 1u);

          if ((Dem_Cfg_GetMasterEvent(lEventId) != lEventId)
            || (Dem_MemState_MemoryGetFirstEvent(DEM_CFG_MEMORYID_PRIMARY) > lEventId)
            || (Dem_MemState_MemoryGetLastEvent(DEM_CFG_MEMORYID_PRIMARY) < lEventId))
          {
  ...
  )
{
  ...
  if ((Dem_UDSStatus_Test_CDTC(Dem_DTC_RetrieveStatus(lEventId)) == FALSE)
    ...
    || ( (ReplaceMisfireOrFuelSys == FALSE)
      && (Dem_Core_Event_TestHighPriorReadinessGroup(lEventId) == FALSE))                                                             
    ))
  {
    lSelectedIndex = 0u;
  }
...
{
  return (Dem_Mem_FreezeFrameIndexMode02 != Dem_Cfg_ObdFFSlotCountInVariant());
}
#endif
