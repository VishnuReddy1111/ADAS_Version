  ...
  )
{
  ...
  boolean lPtoStatus = Dem_GlobalDiagnostics_Pto_GetPtoStatus();
...
)
{
  ...
  if (Dem_Cfg_IsObdSupportedInVariant() == TRUE)
  {
    ...
    if (Dem_FaultMemory_Memory_TranslateDTCOrigin(DTCOrigin, &lTranslatedMemoryId) == E_OK)                                      /* SBSW_DEM_POINTER_FORWARD_STACK */
    {
      ...
      {
        Dem_Iumpr_FilterInfo.FirstEvent = Dem_Memory_GetFirstEvent(lTranslatedMemoryId);
        Dem_Iumpr_FilterInfo.LastEvent = Dem_Memory_GetLastEvent(lTranslatedMemoryId);
        lReturnValue = E_OK;
      }
    }
  }
