...
/* patch version check */
#if (DEM_SW_PATCH_VERSION < 0U)
#error DEM_SW_PATCH_VERSION wrong (< 0U)
#endif
 ...
 ** Check if MILStatusFromPreviousOpCycle must be updated and update the PID21
 ...
 ** entered or updated in event memory after Dem_Init()
 */
VAR(Dem_RootErrorQueueEntryType, DEM_VAR_CLEARED) Dem_RootErrorQueue[DEM_ROOT_ERROR_BUFFER_SIZE];
#endif
...
#endif
      {
...
#endif
        {
          ...
          if (Dem_GbiAgingCycleIdx(EvId) == AgingCycleId)
          {
              ...
              /* !LINKSTO dsn.Dem.InternalAging.DemAgingRequiresTestedCycleSetAgingCycleState.Disabled,1 */
  ...
  boolean WIRStatus)
{
...
#endif /* DEM_DEV_ERROR_DETECT */
  {
...
#endif
    {
      ...
      {
        boolean UserCtrlWIRStatus = FALSE;
        /*
      ...
      (DEM_NUM_CMB_DTCS > 0U) )
        ...
        {
          UserCtrlWIRStatus = TRUE;
        }

        if (WIRStatus != UserCtrlWIRStatus)
        {
          ...
          else /* FASLE is the only possible value at this point */
          {
            /* store the request in array */
        ...
        {
          /* no update is necessary for current event */
        }
...
STATIC FUNC(void, DEM_CODE) Dem_ResetNvData(void)
{
  ...
  Dem_NvData.MILStatusFromPreviousOpCycle = DEM_INDICATOR_OFF;
#endif
  ...
  Dem_EventNotificationDataPtrType Notification)
{
  ...
  if (EventEntryAllocResult != DEM_EVENTENTRYALLOC_INVALID)
  {
    ...
    Dem_ProcessEventEntryJ1939FFData(EventId, EvMemEntryPtr);
...
STATIC FUNC(void, DEM_CODE) Dem_ProcessDistanceRelatedPIDs(void)
{
  ...
  if (Dem_MainFunctionOneSecondCounter == DEM_MAINCYCLE_TICKS_PER_SECOND)
  {
#if (DEM_OBD_CENTRALIZED_PID21_HANDLING == STD_ON)
    const Dem_IndicatorStatusType MILCurrentStatus =
      Dem_InternalGetIndicatorStatus(DEM_MIL_ID, FALSE);
#endif
    ...
    if ((Dem_NvData.MILStatusFromPreviousOpCycle == DEM_INDICATOR_OFF) &&
        (MILCurrentStatus != DEM_INDICATOR_OFF))
    {
    ...
    Dem_NvData.MILStatusFromPreviousOpCycle = MILCurrentStatus;
