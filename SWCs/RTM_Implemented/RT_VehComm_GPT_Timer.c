 ...
 *  -------------------------------------------------------------------------------------------------------------------
 *             File:  RT_VehComm.c
 *           Config:  D:/SIPs/CARIAD/SecOC/02_EcuCfg/ADAS.dpa
 *        SW-C Type:  RT_VehComm
 *  Generation Time:  2025-02-11 16:05:20
 *
 *        Generator:  MICROSAR RTE Generator Version 4.35.0
 *                    RTE Core Version 4.35.0
 *          License:  CBD2201090
 *
...
#define COM_ENABLE 1
#define DEM_ENABLE 0
#define LCW_CODE_ENABLE 1
#define TURN_INDICATOR_CODE_ENABLE 1
#define SFD_ENABLE 0
#define SOK_ENABLE 1
#define INDICATOR_FREQ 5
#if COM_ENABLE
Signal_MyECU_SecOC_Tx2 send_Data = 0xB7;
static uint8 receiveTurnData_u8 = 0xFF;
static uint16 receiveData_u16 = 0xFF;
static uint8 counter_u8 = 0;
static uint16 counter_u16 = 0;
 ...
 * KN_Hybrid_01_BusKnockOut: Integer in interval [0...3]
 * KN_Hybrid_01_BusKnockOutTimer: Integer in interval [0...255]
 * KN_Hybrid_01_ECUKnockOut: Integer in interval [0...3]
 * KN_Hybrid_01_ECUKnockOutTimer: Integer in interval [0...63]
 ...
 *********************************************************************************************************************/
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<1; counter_u8++)
		{
			Data[counter_u8] = 0x05;
		}
 ...
 *********************************************************************************************************************/
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<=23; counter_u8++)
		{
			Data[counter_u8] = counter_u8;
		}
		/* Read Data success, return OK */
		retval_u8 = RTE_E_OK;
	}
 ...
 *********************************************************************************************************************/
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<=104; counter_u8++)
		{
			Data[counter_u8] = 0x0BU;
		}
		/* Read Data success, return OK */
		retval_u8 = RTE_E_OK;
	}
 ...
 *********************************************************************************************************************/
	...
	if(retVal_u8 == E_OK && Data != NULL_PTR)
	{
 ...
 *********************************************************************************************************************/
	...
	if (NULL_PTR != Data)
	{
		for (counter_u16=0; counter_u16<=255; counter_u16++)
		{
			Data[counter_u16] = counter_u16;
		}
		/* Read Data success, return OK */
		retval_u8 = RTE_E_OK;
	}
 ...
 *********************************************************************************************************************/
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<2; counter_u8++)
		{
			Data[counter_u8] = counter_u8;
		}
		/* Read Data success, return OK */
 ...
 *********************************************************************************************************************/
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<20; counter_u8++)
		{
			Data[counter_u8] = counter_u8;
		}
		/* Read Data success, return OK */
 ...
 *********************************************************************************************************************/
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<4; counter_u8++)
		{
			Data[counter_u8] = counter_u8;
		}
		/* Read Data success, return OK */
 ...
 *********************************************************************************************************************/
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<3; counter_u8++)
		{
			Data[counter_u8] = counter_u8;
		}
		/* Read Data success, return OK */
 ...
 *********************************************************************************************************************/
  ...
  /* 0xF1AF: Local Variable Declaration */
	...
	if (NULL_PTR != Data)
	{
		for (counter_u16=0; counter_u16<=516; counter_u16++)
		{
			Data[counter_u16] = 0x05;
		}
		/* Read Data success, return OK */
		retval_u8 = RTE_E_OK;
	}
 ...
 *********************************************************************************************************************/
  ...
  /* 0xF1AF: Local Variable Declaration */
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<=25; counter_u8++)
		{
			Data[counter_u8] = counter_u8;
		}
		/* Read Data success, return OK */
 ...
 *********************************************************************************************************************/
  ...
  /* 0xF1AF: Local Variable Declaration */
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<=22; counter_u8++)
		{
			Data[counter_u8] = counter_u8;
		}
		/* Read Data success, return OK */
 ...
 *********************************************************************************************************************/
  ...
  /* 0xF1AF: Local Variable Declaration */
	...
	if (NULL_PTR != Data)
	{
		for (counter_u8=0; counter_u8<=10; counter_u8++)
		{
			Data[counter_u8] = counter_u8;
		}
		/* Read Data success, return OK */
 ...
 *********************************************************************************************************************/
	...
	(void)Rte_Write_RT_VehComm_PP_BswM_MDGP_ESH_RunRequest_requestedMode(1U);
 ...
 *   Std_ReturnType Rte_Read_RP_WFS_VIN_1_DE_VIN_1(uint8 *data)
 *
 * Output Interfaces:
 * ==================
 *   Explicit S/R API:
 ...
 *   Std_ReturnType Rte_Write_KN_Hybrid_01_BusKnockOut_KN_Hybrid_01_BusKnockOut(KN_Hybrid_01_BusKnockOut data)
 *   Std_ReturnType Rte_Write_KN_Hybrid_01_BusKnockOutTimer_KN_Hybrid_01_BusKnockOutTimer(KN_Hybrid_01_BusKnockOutTimer data)
 *   Std_ReturnType Rte_Write_KN_Hybrid_01_ECUKnockOut_KN_Hybrid_01_ECUKnockOut(KN_Hybrid_01_ECUKnockOut data)
 *   Std_ReturnType Rte_Write_KN_Hybrid_01_ECUKnockOutTimer_KN_Hybrid_01_ECUKnockOutTimer(KN_Hybrid_01_ECUKnockOutTimer data)
 ...
 *********************************************************************************************************************/
uint32 RTComm_Counter_Start,RTComm_Counter_End,RTComm_Counter_Diff[100];
uint8 r=0;
SG_BMC_HV_01 HV_01;
SG_BMC_HV_02 HV_02;
/**********************************************************************************************************************
 ...
 *********************************************************************************************************************/
     ...
     Gpt_StartTimer(0,65535);  // Starting the timer
    
	RTComm_Counter_Start = Gpt_GetTimeElapsed(0); // Getting the start counter in the form of Ticks 
	...
	static uint8 receiveLaneChange_u8 = 0x00;

#if LCW_CODE_ENABLE
	static uint8 DIO_LevelRt_u8 = 0x01;
	static uint8 DIO_LevelLf_u8 = 0x01;
	static uint8 leftLedWarning = 0x0A;
	static uint8 rightLedWarning = 0x00;
	static uint32 lLeftTimerVar_u32 = 0x00;
	static uint32 lRightTimerVar_u32 = 0x00;
	static uint8 RcvdLedWarning = 0x00;
#endif
	...
	(void)Rte_Call_SfdAVersionInfo_GetVersionInfo(&VersionInfoSFD);
	(void)Rte_Switch_SP_BasicSFD_BasicSFDRole(RTE_MODE_BasicSFD_ALLOW);
	(void)Rte_Switch_SP_E2eSFD_E2eSFDRole(RTE_MODE_E2eSFD_ALLOW);
	(void)Rte_Switch_SP_EptiSFD_EptiSFDRole(RTE_MODE_EptiSFD_ALLOW);
	(void)Rte_Switch_SP_ExtendedSFD_ExtendedSFDRole(RTE_MODE_ExtendedSFD_ALLOW);
	(void)Rte_Switch_SP_OtaSFD_OtaSFDRole(RTE_MODE_OtaSFD_ALLOW);
	(void)Rte_Switch_SP_ProductionSFD_ProductionSFDRole(RTE_MODE_ProductionSFD_ALLOW);
	(void)Rte_Switch_SP_SuperUserSFD_SuperUserSFDRole(RTE_MODE_SuperUserSFD_ALLOW);
...
if(SendChallenge == 1)
{
	...
	HV_01.BMC_HV_01_CRC = 0x01;
	// Rte_Write_RT_VehComm_SG_BMC_HV_01_SG_BMC_HV_01(&HV_01);
	...
	(void)Rte_Write_IF_BMC_CMC_Zellspannung_079_DE_BMC_CMC_Zellspannung_079(send_Data);

	(void)Rte_Write_IF_BMC_EnergieVorhalt_DE_BMC_EnergieVorhalt(Writedata_u16);

	(void)Rte_Read_IF_VDSO_Vx3d_DE_VDSO_Vx3d(&receiveData_u16);
	
	(void)Rte_Read_RP_BJB_CALID_Index_01_DE_BJB_CALID_Index_01(&receiveLaneChange_u8);
...
/******************************************************************************************************
                                          Demo Code
*******************************************************************************************************/
#if TURN_INDICATOR_CODE_ENABLE
	//receiveData = (uint8)(receiveData_u16 >> 4u);
	receiveTurnData_u8 = (uint8)(receiveData_u16 >> 4u);

	if(0xA5 == receiveTurnData_u8)
	{
	...
	else if (0x5A == receiveTurnData_u8)
	{
		/*Turn Left Indicator*/
		if(0x00 == tempVal_u8)
		{
	...
	else if (0xAA == receiveTurnData_u8)
	{
		/*Hazard Lamp ON*/
		if(0x00 == tempVal_u8)
		{
	...
	else if (0x55 == receiveTurnData_u8)
	{
		/*Headlamp ON*/
		Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_2, 0u);
		Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_3, 0u);
	}
	else if (0x44 == receiveTurnData_u8)
	{
		/*Headlamp OFF*/
		Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_2, 1u);
		Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_3, 1u);
	}
	else if (0x33 == receiveTurnData_u8)
	{
		/*Indicator OFF*/
		Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_1, 1u);
		Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_4, 1u);
	}
	else if (receiveTurnData_u8 == 0xE8) //BusKnockOut with BEM State TRUE - BusKnockOut should trigger
	{
		(void)Rte_Call_RP_CddNpmGen2_K15State_SetK15State(FALSE);
		(void)Rte_Call_RP_CddNpmGen2_BEM3State_SetBEM3State(TRUE);
	}
	else if (receiveTurnData_u8 == 0xE9)  //EcuKnockOut
	{
		(void)Rte_Call_RP_CddNpmGen2_K15State_SetK15State(FALSE);
	...
	else if (receiveTurnData_u8 == 0xFF) 
	{
		(void)Rte_Call_RP_CddNpmGen2_K15State_SetK15State(TRUE);
	}

	/*Test Pattern*/
	if(0xFE == receiveTurnData_u8)
	{
...
/******************************************************************************************************
										 Start of ADAS Demo Code
*******************************************************************************************************/
#if LCW_CODE_ENABLE
	RcvdLedWarning = receiveLaneChange_u8;

	if (RcvdLedWarning < 4)
	{
		leftLedWarning = RcvdLedWarning;
	}
	else
	{
		rightLedWarning = RcvdLedWarning;
	}

	if (lLeftTimerVar_u32 > 15)
	{
		lLeftTimerVar_u32 = 0;
	}
	else
	{
		if (0x1U == leftLedWarning)
		{
			if (10 == lLeftTimerVar_u32)
			{
				DIO_LevelLf_u8 = DIO_LevelLf_u8 ^ 0x01;
				Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_4, DIO_LevelLf_u8);
				lLeftTimerVar_u32 = 0;
			}
		}
		else if (0x2U == leftLedWarning)
		{
			if (5U == lLeftTimerVar_u32)
			{
				DIO_LevelLf_u8 ^= 0x01;
				Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_4, DIO_LevelLf_u8);
				lLeftTimerVar_u32 = 0x00;
			}
		}
		else if (0x3U == leftLedWarning)
		{
			if ((lLeftTimerVar_u32 == 1U) && (0x5A == receiveTurnData_u8))
			{
				DIO_LevelLf_u8 ^= 0x01;
				Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_4, DIO_LevelLf_u8);
				Dio_WriteChannel(DioConf_DioChannel_DioChannel_Buzzer, DIO_LevelLf_u8);
				lLeftTimerVar_u32 = 0x00;
			}
			else
			{
		...
		else if (0x0U == leftLedWarning && (0x5A != receiveTurnData_u8) && (0xAA != receiveTurnData_u8))
		{
			DIO_LevelLf_u8 = 0x01;
			Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_4, DIO_LevelLf_u8);
			lLeftTimerVar_u32 = 0x00;
	...
	if (lRightTimerVar_u32 > 15)
	{
		lRightTimerVar_u32 = 0;
	}
	else
	{
		if (0x5U == rightLedWarning)
		{
			if (lRightTimerVar_u32 == 10U)
			{
				DIO_LevelRt_u8 ^= 0x01;
				Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_1, DIO_LevelRt_u8);
				lRightTimerVar_u32 = 0x00;
			}
		}
		else if (0x6U == rightLedWarning)
		{
			if (lRightTimerVar_u32 == 5U)
			{
				DIO_LevelRt_u8 ^= 0x01;
				Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_1, DIO_LevelRt_u8);
				lRightTimerVar_u32 = 0x00;
		...
		else if (0x7U == rightLedWarning)
		{
			if ((lRightTimerVar_u32 == 1U) && (0xA5 == receiveTurnData_u8))
			{
				DIO_LevelRt_u8 ^= 0x01;
				Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_1, DIO_LevelRt_u8);
				Dio_WriteChannel(DioConf_DioChannel_DioChannel_Buzzer, DIO_LevelRt_u8);
				lRightTimerVar_u32 = 0x00;
			}
			else
			{
		...
		else if (0x4U == rightLedWarning && (0xA5 != receiveTurnData_u8) && (0xAA != receiveTurnData_u8))
		{
			DIO_LevelRt_u8 = 0x01;
			Dio_WriteChannel(DioConf_DioChannel_DioChannel_LED_1, DIO_LevelRt_u8);
			lRightTimerVar_u32 = 0x00;
			rightLedWarning = 0x00;
		}
		else
		{
			lRightTimerVar_u32 = 0x00;
...
RTComm_Counter_End = Gpt_GetTimeElapsed(0);  // Getting the stop counter in the form of Ticks 

RTComm_Counter_Diff[r] = (RTComm_Counter_End - RTComm_Counter_Start);
r++;

if(r==100)   // Taking 100 samples 
{
  r=0;
}
 Gpt_StopTimer(0,65535); // Stopping the timer
	/******************************************************************************************************
											 End of ADAS Demo Code
	*******************************************************************************************************/
...
/**********************************************************************************************************************
 *
 * Runnable Entity Name: SokFmCrTransmitterConfig_BMC_HV_02_ChallengeReceivedIndication
 *
 *---------------------------------------------------------------------------------------------------------------------
 *
 * Executed if at least one of the following trigger conditions occurred:
 *   - triggered by server invocation for OperationPrototype <ChallengeReceivedIndication> of PortPrototype <SokFmCrTransmitterConfig_BMC_HV_02>
 *
 **********************************************************************************************************************
 *
 * Runnable prototype:
 * ===================
 *   Std_ReturnType SokFmCrTransmitterConfig_BMC_HV_02_ChallengeReceivedIndication(void)
 *
 **********************************************************************************************************************
 *
 * Available Application Errors:
...
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of documentation area >>                  DO NOT CHANGE THIS COMMENT!
 * Symbol: SokFmCrTransmitterConfig_BMC_HV_02_ChallengeReceivedIndication_doc
 *********************************************************************************************************************/
...
FUNC(Std_ReturnType, RT_VehComm_CODE) SokFmCrTransmitterConfig_BMC_HV_02_ChallengeReceivedIndication(void) /* PRQA S 0624, 3206 */ /* MD_Rte_0624, MD_Rte_3206 */
{
/**********************************************************************************************************************
 * DO NOT CHANGE THIS COMMENT!           << Start of runnable implementation >>             DO NOT CHANGE THIS COMMENT!
 * Symbol: SokFmCrTransmitterConfig_BMC_HV_02_ChallengeReceivedIndication (returns application error)
 *********************************************************************************************************************/

 HV_02.BMC_HV_02_CRC = 0x01;
 HV_02.BMC_AC_Laden  = 0x02;

 Rte_Write_SG_BMC_HV_02_SG_BMC_HV_02(&HV_02);
...
{
    (void)Rte_Write_RT_VehComm_PP_BswM_MDGP_ESH_RunRequest_requestedMode(0);
}
